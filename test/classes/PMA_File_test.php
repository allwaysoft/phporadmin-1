...
class PMA_File_test extends PHPUnit_Framework_TestCase
{
    ...
    public function testContent($file, $mime)
    {
        ...
        $file1 = new PMA_File($file);
        $file1->setDecompressContent(true);
        //print($file1);
        $this->assertTrue($file1->open());
        if ($mime == 'application/zip') {
            $this->assertEquals($orig, $file1->content_uncompressed);
        } else {
            $this->assertEquals($orig, $file1->getNextChunk());
        }
    }
