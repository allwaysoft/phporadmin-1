...
function PMA_exportHeader()
{
    ...
    $head  =  PMA_exportComment('phpOrAdmin SQL Dump')
           .  PMA_exportComment('version ' . PMA_VERSION)
           .  PMA_exportComment('http://?')
           .  PMA_exportComment();
...
function PMA_getTableDef($db, $table, $crlf, $error_url, $show_dates = false, $add_semicolon = true, $view = false)
{
    ...
    $result = PMA_DBI_query('SELECT * FROM ALL_TABLES WHERE OWNER LIKE \'' . ($db) . '\' AND TABLE_NAME LIKE \'' . PMA_sqlAddslashes($table) . '\'', null, PMA_DBI_QUERY_STORE);
    if ($result != FALSE) {
...
*/
    ...
    $result = PMA_DBI_try_query('SELECT DBMS_METADATA.GET_DDL(\'TABLE\', \''. $table . '\', \'' . $db . '\') FROM DUAL');
    ...
    if ($result != FALSE && ($row = PMA_DBI_fetch_row($result))) {
        ...
        //$create_query = $row[1];
        $create_query = $row[0]->read($row[0]->size());
        unset($row);
