...
class PMA_Index
{
    ...
    static protected function _loadIndexes($table, $schema)
    {
        ...
        $_raw_indexes =
     PMA_DBI_fetch_result('SELECT A.*, B.* FROM ALL_INDEXES A, ALL_IND_COLUMNS B WHERE A.TABLE_OWNER LIKE \''
     . ($schema) . '\' AND A.TABLE_NAME LIKE \'' 
     . ($table) . '\' AND A.INDEX_NAME = B.INDEX_NAME');
        foreach ($_raw_indexes as $_each_index) {
            $_each_index['Schema'] = $schema;
            if (! isset(PMA_Index::$_registry[$schema][$table][$_each_index['INDEX_NAME']])) {
                $key = new PMA_Index($_each_index);
                PMA_Index::$_registry[$schema][$table][$_each_index['INDEX_NAME']] = $key;
            } else {
                $key = PMA_Index::$_registry[$schema][$table][$_each_index['INDEX_NAME']];
            }
    ...
    public function set($params)
    {
        ...
        if (isset($params['UNIQUENESS'])) {
            $this->_non_unique = $params['UNIQUENESS'];
        }
        if (isset($params['COMPRESSION'])) {
            $this->_packed = $params['COMPRESSION'];
        }
    ...
    static public function getView($table, $schema, $print_mode = false)
    {
        ...
        $r .= '<th>' . __('INDEX_NAME') . '</th>';
        $r .= '<th>' . __('INDEX_TYPE') . '</th>';
        $r .= '<th>' . __('UNIQUENESS') . '</th>';
        $r .= '<th>' . __('COMPRESSISON') . '</th>';
        $r .= '<th>' . __('COLUMN_NAME') . '</th>';
        $r .= '<th>' . __('COLUMN_POSITION') . '</th>';
        $r .= '<th>' . __('DESCEND') . '</th>';
/*
        $r .= '<th>' . __('Keyname') . '</th>';
        $r .= '<th>' . __('Type') . '</th>';
        ...
        foreach ($indexes as $index) {
            ...
            if (! $print_mode) {
                ...
                if ($index->getName() == 'PRIMARY') {
                    ...
                    $this_params['sql_query'] = ' DROP INDEX ' . PMA_backquote($index->getName());
            ...
            foreach ($index->getColumns() as $column) {
                ...
                $r .= '</td>';
                $r .= '<td>' . htmlspecialchars($column->getSeqInIndex()) . '</td>';
...
class PMA_Index_Column
{
    ...
    public function set($params)
    {
        ...
        if (isset($params['COLUMN_POSITION'])) {
            $this->_seq_in_index = $params['COLUMN_POSITION'];
        }
        if (isset($params['DESCEND'])) {
            $this->_collation = $params['DESCEND'];
        }
