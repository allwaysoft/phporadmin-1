...
{
    return PMA_DBI_fetch_result("SELECT TABLE_NAME FROM ALL_TABLES WHERE OWNER LIKE '" . $database . "'",
...
*/
        $sql = "
           SELECT *
             FROM ALL_TABLES
            WHERE OWNER LIKE '" . $this_databases . "'" . $sql_where_table;
    ...
    if (empty($tables)) {
        foreach ($databases as $each_database) {
            ...
            $each_tables = PMA_DBI_fetch_result($sql, 'TABLE_NAME', null, $link);

            // Sort naturally if the confi . '\''g allows it and we're sorting
            // the Name column.
            if ($sort_by == 'Name' && $GLOBALS['cfg']['NaturalOrder']) {
            ...
            foreach ($each_tables as $table_name => $each_table) {
                ...
                $each_tables[$table_name]['Name']        =& $each_tables[$table_name]['TABLE_NAME'];
                $each_tables[$table_name]['TABLE_NAME']        =& $each_tables[$table_name]['TABLE_NAME'];
    ...
    $column = null, $link = null)
{
    ...
    if (! $GLOBALS['cfg']['Server']['DisableIS']) {
        ...
        $sql = 'SELECT * FROM ALL_TAB_COLUMNS WHERE OWNER LIKE \''
            . ($database) . '\' AND TABLE_NAME LIKE \'' . ($table) . '\'';
        if (null !== $column) {
            $sql .= " LIKE '" . $column . "'";
        }
...
{
    $fields = PMA_DBI_fetch_result(
        'SELECT ' . ($full ? '*' : 'COLUMN_NAME') . ' FROM ALL_TAB_COLUMNS
        WHERE OWNER LIKE \'' . ($database) . '\' AND TABLE_NAME LIKE \''
        . ($table) . '\'',
...
function PMA_DBI_get_table_indexes($database, $table, $link = null)
{
    ...
    $indexes = PMA_DBI_fetch_result(
              'SELECT * FROM ALL_INDEXS WHERE OWNER LIKE \''
              .($database) . '\' AND TABLE_NAME LIKE \'' . ($table) .'\'',
               null, null, $link);
...
function PMA_DBI_get_definition($db, $which, $name, $link = null)
{
    ...
    $query = 'SELECT DBMS_METADATA.GET_DDL(\'TABLE\',\'' . $name . '\', \'' . ($db) . '\') FROM DUAL';
...
function PMA_DBI_get_triggers($db, $table, $delimiter = '//')
{
    ...
    // instead of WHERE EVENT_OBJECT_SCHEMA='dbname'
        ...
        $triggers = PMA_DBI_fetch_result("SELECT * FROM ALL_TRIGGERS WHERE OWNER LIKE '" . PMA_sqlAddslashes($db,true) . "' AND TABLE_NAME LIKE '" . PMA_sqlAddslashes($table, true) . "'");
    }
        $triggers = PMA_DBI_fetch_result("SELECT * FROM ALL_TRIGGERS WHERE OWNER LIKE '" . PMA_sqlAddslashes($db,true) . "' AND TABLE_NAME LIKE '" . PMA_sqlAddslashes($table, true) . "'");
    ...
    if ($triggers) {
        foreach ($triggers as $trigger) {
            ...
            */
            ...
            $one_result['action_timing'] = $trigger['TRIGGER_TYPE'];
            $one_result['event_manipulation'] = $trigger['TRIGGERING_EVENT'];
