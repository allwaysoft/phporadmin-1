...
function PMA_DBI_num_rows1($query)
{
    if ($query) {
        $query = 'SELECT COUNT(*) AS CT from (' . $query . ')';
        $result=PMA_DBI_try_query($query);
        if($result) {
           $ct_row= PMA_DBI_fetch_row($result);
           return (int)$ct_row['CT'];
        } else {
           return 0;
        }
 ...
 */
function PMA_DBI_affected_rows($result = null, $get_from_cache = true)
{
    if (empty($result)) {
            return false;
    }
...
{
    return oci_field_size($result, $i);
}

function PMA_DBI_field_name($result, $i)
{
    return oci_field_name($result, $i);
}
