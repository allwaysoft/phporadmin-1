...
if ((! $cfg['ShowAll'] || $_SESSION['tmp_user_values']['max_rows'] != 'all')
 ...
 && empty($analyzed_sql[0]['limit_clause'])
    ...
    //$row_end = $_SESSION['tmp_user_values']['pos']I/
    $sql_limit_to_append = ' WHERE ROW$NUM BETWEEN '
                         . $_SESSION['tmp_user_values']['pos'] . ' AND '
                         . $_SESSION['tmp_user_values']['pos'] . '-1+'
                         . $_SESSION['tmp_user_values']['max_rows'] . " ";

    $full_sql_query = 'SELECT * FROM ( SELECT ROWNUM AS ROW$NUM, A.* FROM ( '
                    . $analyzed_sql[0]['section_before_limit'] . ' ) A )'
                    . $sql_limit_to_append
...
// At least one row is returned -> displays a table with results
    ...
    if (isset($result) && $result) {
        $fields_meta = PMA_DBI_get_fields_meta($result, $table);
