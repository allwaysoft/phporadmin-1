...
$table_fields = PMA_DBI_fetch_result('SELECT * FROM ALL_TAB_COLUMNS WHERE OWNER LIKE \'' . $db . '\' AND TABLE_NAME LIKE \'' . ($table) . '\'',
...
if (isset($where_clause)) {
    ...
    $insert_mode = true;
    $result = PMA_DBI_query('SELECT * FROM ' . PMA_backquote($db) . '.' . PMA_backquote($table), null, PMA_DBI_QUERY_STORE);
...
<?php
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        ...
        $table_fields[$i]['Field']=$table_fields[$i]['COLUMN_NAME'];
        $table_fields[$i]['Type']=$table_fields[$i]['DATA_TYPE'];
  $table_fields[$i]['Null']=$table_fields[$i]['NULLABLE'];
