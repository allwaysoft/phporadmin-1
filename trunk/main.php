...
$GLOBALS['db'] = '';
//$GLOBALS['db'] = strtoupper($cfg['Server']['user']);
$GLOBALS['table'] = '';
$show_query = '1';
...
if ($server > 0) {
    ...
    $mysql_cur_user_and_host = PMA_DBI_fetch_value('SELECT USERNAME FROM USER_USERS');
...
if ($GLOBALS['cfg']['ShowServerInfo'] || $GLOBALS['cfg']['ShowPhpInfo']) {
    ...
    if ($GLOBALS['cfg']['ShowServerInfo']) {
        ...
        if ($server > 0) {
            PMA_printListItem(__('Oracle C client library version') . ': ' . PMA_DBI_get_client_info(),
                'li_oracle_c_client_version');
